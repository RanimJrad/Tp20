...
public class HomeActivityAdmin extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        logoutIcon.setOnClickListener(v -> {
            Intent intent = new Intent(HomeActivityAdmin.this, Login.class);
        ...
        ItemTouchHelper.SimpleCallback itemTouchHelperCallback = new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
                // Récupérer la position et l'hôtel
                int position = viewHolder.getAdapterPosition();
                Hotel hotel = hotelList.get(position);

                // Supprimer l'hôtel de Firebase
                databaseReference.child(hotel.getId()).removeValue()
                        .addOnCompleteListener(task -> {
                            if (task.isSuccessful()) {
                                // Supprimer l'hôtel de la liste
                                hotelList.remove(position);

                                // Notifier le RecyclerView d'un changement complet
                                hotelAdapterAdmin.notifyDataSetChanged();

                                Log.d("HomeActivity", "Hotel deleted successfully");
                            } else {
                                // Restaurer l'élément s'il y a une erreur
                                hotelAdapterAdmin.notifyItemChanged(position);
